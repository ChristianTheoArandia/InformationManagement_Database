...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($payment_type_id == '003') {
        ...
        // Validate reference number based on wallet type
    ...
    if (!$error) {
        ...
        if ($stmt->execute()) {
            ...
            echo '<script>alert("' . $message . '"); window.location.href = "../transactions/list.php";</script>';
            exit();
        } else {
...
<head>
    ...
    <style>
        * { font-family: 'Poppins', sans-serif; }
        ...
        .btn-payment:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(102,126,234,0.3); }
        .btn-back {
            background: #6b7280;
            color: white;
            border: none;
...
<body>
    ...
    <script>
        // Update reference number hint based on wallet type
